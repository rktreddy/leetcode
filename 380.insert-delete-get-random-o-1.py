...
# @lc code=start
# class RandomizedSet:
#     """ Approach 1: HashMap + ArrayList O(1), O(N) """
...
class RandomizedSet:
    """ practice: Approach 1: HashMap + ArrayList O(1), O(N) """
    def __init__(self):
        self.dict = {}
        self.list = []
    ...
    def remove(self, val):
        if val not in self.dict:
            ...
            self.list[cur_idx], self.list[-1] = last_element, val
            self.dict[last_element] = cur_idx
            self.list.pop()
            del self.dict[val]
            return True
